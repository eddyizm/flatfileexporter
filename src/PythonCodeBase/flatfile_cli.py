...
log_handler = RotatingFileHandler(
    LOG_FILE, mode="a", maxBytes=5 * 1024 * 1024, backupCount=5, encoding=None, delay=0
)
...
parser = argparse.ArgumentParser(
    description="Reads a sql script or calls a stored procedure and exports results to a csv, txt or excel file."
)
...
parser.add_argument(
    "directory", help="location to export flat file. do not specify extension."
)
# optionals
parser.add_argument(
    "-f", "--filename", help="filename. Date will be appended.", required=False
)
parser.add_argument(
    "-u", "--username", help="username for db connection.", default=None, required=False
)
parser.add_argument(
    "-pass",
    "--password",
    help="use login/pass from instead of default Integrated Security (Trusted_Connection).",
    default=None,
    required=False,
)
# real first group for extension
group_extension = parser.add_mutually_exclusive_group(required=True)
group_extension.add_argument(
    "-csv",
    "--csv",
    action="store_const",
    const=".csv",
    help="output file with csv extension.",
)
group_extension.add_argument(
    "-txt",
    "--txt",
    action="store_const",
    const=".txt",
    help="output file with txt extension.",
)
group_extension.add_argument(
    "-xlsx", "--xlsx", action="store_const", const=".xlsx", help="output excel file."
)
# first group for seperator
group_seperator = parser.add_mutually_exclusive_group(required=True)
group_seperator.add_argument(
    "-c",
    "--comma",
    action="store_const",
    const=",",
    help="use comma character for seperator.",
)
group_seperator.add_argument(
    "-t", "--tab", action="store_const", const="\t", help="use tab for seperator."
)
group_seperator.add_argument(
    "-p",
    "--pipe",
    action="store_const",
    const="|",
    help="use pipe character for seperator.",
)
# second group for sql script or stored proc
group_sql = parser.add_mutually_exclusive_group(required=True)
group_sql.add_argument(
    "-s",
    "--sqlscript",
    help="full path of sql script *MUST HAVE* SET NO COUNT on script.",
)
group_sql.add_argument(
    "-sp",
    "--storedproc",
    help="instead of sql script, point to stored procedure on database. Wrap in double quotes with parameters eg. \"EXEC SAMPLE_SP '<Param>'\"",
)
